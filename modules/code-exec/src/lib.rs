//! code-exec: a slim-m WASM module that runs a JavaScript snippet with boa_engine.
//!
...
//! and imports nothing. The module never traps on user JS errors; it always
//! answers with `{"ok":true,"output":...}` or `{"ok":false,"error":...}`.
...
/// pointer to them. Called by the host before `run`, to get somewhere to
...
fn handle(request: &[u8]) -> Vec<u8> {
    ...
    match runtime::execute(&parsed.input) {
