//! A deterministic splitmix64 counter, seeded from bytes or from a raw seed.
//!
...
impl SplitMix64 {
    ...
    /// Seeds from an existing u64, for a board that has to be rebuilt
    /// identically on every frame from the seed carried in its state.
