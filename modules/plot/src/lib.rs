//! plot: a slim module that renders a bar chart from numbers, as a scene.
//! `lib.rs` is the ABI shim; the charting lives in `chart.rs`.
//!
...
fn handle(request: &[u8]) -> Vec<u8> {
    ...
    match chart::apply(&parsed.command, &parsed.input) {
